...
from task_app.serializer import CitizenCreateSerializer


class ImportsTestCase(TestCase):
    def setUp(self):
        with open('task_app/tests/data.json', 'r', encoding='utf-8') as file:
            self.data = json.load(file)

    def test_post_imports(self):
        response = self.client.post('/imports', data=self.data, content_type='application/json')
...
class CitizenUpdateTest(TestCase):
    def setUp(self) -> None:
        serializer = CitizenCreateSerializer(data=set1)
